...
class FrontendStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    createFrontendAliasRecords(this, environment, frontend, distribution);
...
function createFrontendResponseHeadersPolicy(scope, environment) {
  return new cloudfront.ResponseHeadersPolicy(scope, "FrontendResponseHeadersPolicy", {
    ...
    comment: `Security headers for Lynx Pardelle Angular SSR frontend (${environment.name}).`,
    removeHeaders: ["X-Powered-By"],
    securityHeadersBehavior: {
      contentSecurityPolicy: {
        contentSecurityPolicy: [
          "default-src 'self'",
          "script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://www.youtube.com https://www.youtube-nocookie.com https://www.dailymotion.com https://*.dmcdn.net https://pagead2.googlesyndication.com",
          "style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net",
          "img-src 'self' data: blob: https:",
          "font-src 'self' data: https://cdn.jsdelivr.net",
          "connect-src 'self' https://api.lynxpardelle.com https://assets.lynxpardelle.com",
          "media-src 'self' blob: https://api.lynxpardelle.com https://assets.lynxpardelle.com",
...
function createFrontendAliasRecords(scope, environment, frontend, distribution) {
  if (!frontend.route53RecordsEnabled || !frontend.domainName) {
    return;
  }

  const domainNames = frontendAliasDomainNames(frontend);
  if (frontend.route53RecordManagement === "upsert") {
    createFrontendAliasUpsert(scope, environment, domainNames, distribution);
    return;
  }

  const hostedZone = route53.HostedZone.fromHostedZoneAttributes(scope, "FrontendHostedZone", {
    hostedZoneId: environment.hostedZoneId,
    zoneName: environment.hostedZoneName,
  });
  for (const [index, domainName] of domainNames.entries()) {
    const idSuffix = index === 0 ? "" : pascalId(domainName);
    new route53.ARecord(scope, `FrontendAliasARecord${idSuffix}`, {
      zone: hostedZone,
      recordName: domainName,
      target: route53.RecordTarget.fromAlias(new targets.CloudFrontTarget(distribution)),
    });
    new route53.AaaaRecord(scope, `FrontendAliasAaaaRecord${idSuffix}`, {
      zone: hostedZone,
      recordName: domainName,
      target: route53.RecordTarget.fromAlias(new targets.CloudFrontTarget(distribution)),
    });
...
function createFrontendAliasUpsert(scope, environment, domainNames, distribution) {
  const changeBatch = {
    Comment: `Upsert Lynx Pardelle frontend aliases for ${environment.name}.`,
  ...
  const sdkCall = {
    ...
    physicalResourceId: cr.PhysicalResourceId.of(
      `${buildResourceName(environment, "frontend", "aliases")}-${domainNames.map(pascalId).join("-")}`
    ),
  };

  new cr.AwsCustomResource(scope, "FrontendAliasUpsert", {
    resourceType: "Custom::PortfolioFrontendAliasRecords",
    onCreate: sdkCall,
    onUpdate: sdkCall,
...
function frontendAliasDomainNames(frontend) {
  return [...new Set([frontend.domainName, ...(frontend.alternateDomainNames || [])].filter(Boolean))];
}
